...
async function handleLogin() {
  ...
  try {
    ...
    error.value = Array.isArray(detail) ? detail.map((d: any) => d.msg).join('; ') : (detail || '登录失败')
  }
}
</script>
...
<template>
  <div class="auth-container">
    <div class="auth-card">
      <h1 class="logo">聚流</h1>
      <p class="subtitle">JuFlow — 国内博主聚合阅读器</p>
      ...
      <p class="link">{{ t('auth.noAccount') }}<router-link to="/register">{{ t('auth.register') }}</router-link></p>
    </div>
  </div>
</template>
